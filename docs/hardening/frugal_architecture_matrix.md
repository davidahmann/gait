...
| Use managed simplicity where possible | Leans on standard crypto/libs and GitHub Actions instead of bespoke infra | `core/sign/*`, `.github/workflows/*`, `Makefile` | Keep: boring primitives. Do next: pin additional workflow dependencies for reproducibility. Avoid: custom crypto or custom scheduler layers. |
