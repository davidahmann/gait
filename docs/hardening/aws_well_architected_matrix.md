...
| Security | Signed artifacts/traces, credential broker hardening, allowlist controls, vulnerability scanning | `core/sign/*`, `core/credential/providers.go`, `Makefile` (`gosec`, `govulncheck`, `bandit`) | Add threat model doc for broker and policy bypass scenarios with mitigations. |
...
| Performance Efficiency | Deterministic zip/canonicalization routines and benchmark tooling | `core/zipx/zipx.go`, `core/jcs/jcs.go`, `perf/`, `Makefile` (`bench`) | Add automated benchmark regression gate on scheduled CI with alert threshold. |
