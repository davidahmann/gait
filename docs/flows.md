...
## 6) v2.1 Additive Flow (Planned Session + Delegation Extensions)

```mermaid
sequenceDiagram
    participant Runtime as Agent Runtime
    participant Gate as gait boundary
    participant Journal as Session Journal (planned)
    participant Verify as Session Chain Verify (planned)

    Runtime->>Gate: intent (+ context.session_id today)
    Gate-->>Runtime: verdict + trace
    Note over Runtime,Gate: Current OSS path (shipped)
    Runtime->>Journal: append checkpoint event (planned additive)
    Journal-->>Runtime: checkpoint artifact ref
    Runtime->>Verify: verify session chain continuity (planned additive)
    Verify-->>Runtime: ok/fail with stable reason code
```

Notes:

- Session/delegation chain flows above are planned additive extensions, not required for current v1 operation.
- Current integrations should still populate context fields now so later adoption is incremental.
